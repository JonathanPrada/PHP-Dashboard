    ...
    <div class="input-group search-field">
        ...
        <input id="checklist-list" type="hidden" name="savedlist" value="">
            <span class="input-group-btn">
                <input id="submit-form" type="submit" class="hidden" name="checklist-submit" value="Spreichern" onclick="listGrabber()"/>
            </span>
    </div>
</form>
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $getname = $_POST['name'];
    //filter it
    $name = filter_var($getname, FILTER_SANITIZE_STRING, HTML_SPECIALCHARS);

    //do simple validation
    if ($name == "") {
        echo "You must include a valid name for the list";
    } else {
        ...
        try {
            ...
            $sql = $conn->query("SELECT listid FROM list WHERE listname = ('$name')");
